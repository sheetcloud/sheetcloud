...
ENV_SHEETCLOUD_API_URL = os.environ.get(
...
ENV_SHEETCLOUD_DEV = (
    str(os.environ.get("SHEETCLOUD_DEV", "False")).strip().lower() == "true"
)
...
logger.info(
    f'Environment variable SHEETCLOUD_PASSWORD is {pw_obfuscator(ENV_SHEETCLOUD_PASSWORD, "missing")}.'
)
if ENV_SHEETCLOUD_LICENSE is not None:
    logger.info(
        f'(Optional) Environment variable SHEETCLOUD_LICENSE is {pw_obfuscator(ENV_SHEETCLOUD_LICENSE, "missing")}.'
    )
    ...
    while num_retries > 0:
        if method == "post":
            response = requests.post(
                f"{ENV_SHEETCLOUD_API_URL}{path}",
                data=payload,
                params=params,
                files=files,
                headers=headers,
                timeout=100,
                verify=not ENV_SHEETCLOUD_DEV,
            )
        else:
        ...
        if response.status_code == 400:
            # file/worksheet/spreadsheet does not exist
            return f"Error while retrieving data. Does the sheet/file exist?"
...
def request_auth_token(num_retries: int = 3) -> Optional[str]:
    ...
    while num_retries > 0:
        response = requests.post(
            f"{ENV_SHEETCLOUD_API_URL}/token",
            data,
            headers=headers,
            timeout=100,
            verify=not ENV_SHEETCLOUD_DEV,
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 401:
                logger.error(
                    f"Authorization failed. Check your user name and password again."
                )
                return None
            if response.content is not None:
                ...
                if "detail" in err_msg:
                    logger.error(
                        f'Authorization failed with "{err_msg["detail"]}". Trying again...'
                    )
        time.sleep(3)
        num_retries -= 1
    logger.error(
        f"Authorization failed (user={ENV_SHEETCLOUD_USERNAME}, pw={pw_obfuscator(ENV_SHEETCLOUD_PASSWORD)})."
    )
    return None

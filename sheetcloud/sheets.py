...
logging.basicConfig(
    format="\x1b[38;5;224m %(levelname)8s \x1b[0m | \x1b[38;5;39m %(name)s \x1b[0m | %(message)s",
    level=logging.DEBUG,
)
...
def get_modified_datetime(sheet_url_or_name: str) -> Tuple[datetime, str, str]:
    ...
    logger.warning(
        f"Could not find timestamp of speadsheet {sheet_url_or_name}. Returning current time."
    )
    return datetime.now(), None, None


def format_spreadsheet(
    sheet_url_or_name: str,
    worksheet_name: str,
    a1range_format_list: List[Tuple[str, Dict]],
    ...
    print(fmts)
    params = {
        "spreadsheet_url_or_name": sheet_url_or_name,
...
def share(
    sheet_url_or_name: str,
    share_emails_write_access: Optional[List[str]] = None,
    share_emails_read_only_access: Optional[List[str]] = None,
    notification_msg: Optional[str] = None,
) -> None:
    params = {"spreadsheet_url_or_name": sheet_url_or_name}
    data = {
        "emails_write_access": share_emails_write_access,
        "emails_read_access": share_emails_read_only_access,
...
def read(
    ...
    content = service(
        "/sheets/read", params=params, headers=headers, method="post", return_dict=False
    )
    if isinstance(content, dict) or isinstance(content, str):
        logger.error(content)
        return None
    df = pd.read_parquet(
        io.BytesIO(content), engine="pyarrow", use_nullable_dtypes=True
    )
...
def write(
    ...
    """Write a DataFrame to a specific worksheet within a given spreadsheet. If the spreadsheet `sheet_url_or_name` does not exist, create a
    ...
    with io.BytesIO() as memory_buffer:
        df.to_parquet(memory_buffer, compression="gzip", engine="pyarrow")
        memory_buffer.seek(0)
        # need to send files separately
        files = {"file": ("Test", memory_buffer, "application/octet-stream")}
        params = {
            "spreadsheet_url_or_name": sheet_url_or_name,
            "worksheet_name": worksheet_name,
        }
...
def append(
    sheet_url_or_name: str, worksheet_name: str, df: pd.DataFrame, cache: bool = True
) -> None:
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(
        [[1, 2, 3], [4, pd.NA, 6], [7, 7, pd.NA]], columns=["col1", "col2", "col3"]
    )
